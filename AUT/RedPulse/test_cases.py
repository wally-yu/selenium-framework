...
def MultiUserInteractionDemo(url):
    ...
    driver_normal_user = start_browser(url)
    time.sleep(settings.T_MINI)
    ...
    driver_supervisor = start_browser(url)
    time.sleep(settings.T_MINI)
    ...
    Login().login(driver_normal_user, 'testing@redpulse.com', 'mvp4ever')

    # supervisor login
    switch_current_window_to_front(driver_supervisor)
    landing_action_obj.nagivate_to_login(driver_supervisor)
    Login().login(driver_supervisor, 'test@red-pulse.com', '123')
